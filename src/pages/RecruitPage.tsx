  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        <span style={{ color: '#06b6d4', fontWeight: 700, fontSize: 20 }}>Recruit - {encounter?.name ?? 'Wild Champion'}</span>
        ...
        {!result ? (
          <>
            <div style={previewCardStyle}>
              <img src={champ?.iconUrl ?? ''} alt={champ?.name ?? '???'} style={iconStyle}
                onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#e6edf3', marginBottom: 4 }}>
                  {champ?.name ?? encounter?.championId ?? '???'}
                </div>
                <div style={{ fontSize: 14, color: '#8b949e', marginBottom: 8 }}>{champ?.title ?? 'Champion'}</div>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginBottom: 12 }}>
                  {champ?.tags.map(tag => (<span key={tag} style={tagStyle}>{tag}</span>))}
                </div>
                {champ && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 12, color: '#8b949e' }}>
                    <div>HP: <span style={{ color: '#22c55e' }}>{Math.round(champ.stats.hp)}</span></div>
                    <div>AD: <span style={{ color: '#ef4444' }}>{Math.round(champ.stats.attackDamage)}</span></div>
                    <div>Armor: <span style={{ color: '#3b82f6' }}>{Math.round(champ.stats.armor)}</span></div>
                    <div>MR: <span style={{ color: '#a855f7' }}>{Math.round(champ.stats.magicResist)}</span></div>
                    <div>AS: <span style={{ color: '#facc15' }}>{champ.stats.attackSpeed.toFixed(2)}</span></div>
                    <div>CRIT: <span style={{ color: '#f97316' }}>{Math.round(champ.stats.crit)}%</span></div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <button style={{ ...recruitBtnStyle, opacity: disabled ? 0.4 : 1, cursor: disabled ? 'not-allowed' : 'pointer' }}
                onClick={handleRecruit} disabled={disabled}>{label}</button>
              <button style={leaveBtnStyle} onClick={handleLeave}>Pass</button>
            </div>
          </>
        ) : (
          <>
            <div style={{ fontSize: 48, marginBottom: 16 }}>{result === 'success' ? '🎉' : '🚫'}</div>
            <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 12, color: result === 'success' ? '#22c55e' : '#ef4444' }}>
              {result === 'success' ? (champ?.name ?? 'Champion') + ' joined your team!' : (champ?.name ?? 'Champion') + ' fled!'}
            </div>
            <div style={{ fontSize: 14, color: '#c8aa6e', marginBottom: 24, textAlign: 'center', maxWidth: 400 }}>
              {result === 'success' ? 'Spent ' + (encounter?.cost ?? 0) + 'g.' : 'The champion ran away. You keep your gold.'}
            </div>
            <button style={continueBtnStyle} onClick={handleLeave}>Continue</button>
          </>
        )}
      </div>
    </div>
...
const previewCardStyle: React.CSSProperties = {
  background: '#161b22', border: '1px solid #2d333b', borderRadius: 12,
  padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center',
  gap: 12, marginBottom: 16, minWidth: 260,
};
const iconStyle: React.CSSProperties = {
  width: 72, height: 72, borderRadius: 8, background: '#1e2a3a',
};
const tagStyle: React.CSSProperties = {
  padding: '2px 10px', background: '#1e2a3a', border: '1px solid #2d333b',
  borderRadius: 12, fontSize: 11, color: '#7dd3fc',
};
const recruitBtnStyle: React.CSSProperties = {
  padding: '14px 36px', background: '#06b6d4', color: '#fff',
  border: 'none', borderRadius: 8, fontSize: 16, fontWeight: 700, cursor: 'pointer',
};
const leaveBtnStyle: React.CSSProperties = {
  padding: '14px 36px', background: '#21262d', color: '#c8aa6e',
