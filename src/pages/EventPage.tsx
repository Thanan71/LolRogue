...
export function EventPage() {
    ...
    switch (resolved.type) {
      ...
      case 'heal': {
        ...
        const updates = state.team.map(member => {
          ...
          return { championId: member.championId, currentHp: Math.min(maxHp, currentHp + Math.floor(maxHp * healPct)),
            level: member.level ?? 1, currentXp: member.currentXp ?? 0 };
        });
      ...
      case 'damage': {
        ...
        const updates = state.team.map(member => {
          ...
          const maxHp = champ?.stats.hp ?? 100;
          const currentHp = member.currentHp ?? maxHp;
          return { championId: member.championId, currentHp: Math.max(1, currentHp - Math.floor(currentHp * dmgPct)),
            level: member.level ?? 1, currentXp: member.currentXp ?? 0 };
        });
        state.updateTeamAfterCombat(updates);
        break;
      ...
      case 'item_reward': {
        if (resolved.item) {
          addItem({ id: resolved.item.itemId, name: resolved.item.name, description: resolved.item.description,
            iconUrl: resolved.item.iconUrl, stats: resolved.item.stats, passiveId: resolved.item.passiveId,
  ...
  const typeColors: Record<string, string> = {
    gold_reward: '#ffd700', gold_cost: '#ef4444', item_reward: '#7dd3fc',
    heal: '#22c55e', damage: '#ef4444', champion_recruit: '#06b6d4',
    stat_boost: '#a855f7', nothing: '#8b949e',
  };
  const typeIcons: Record<string, string> = {
    gold_reward: '\u{1f4b0}', gold_cost: '\u{1f4b8}', item_reward: '\u{1f381}',
    heal: '\u{1f49a}', damage: '\u{1f4a5}', champion_recruit: '\u{1f91d}',
    stat_boost: '\u2b06\ufe0f', nothing: '\u{1f4a8}',
  };
      ...
      <div style={contentStyle}>
        {!outcome ? (
          <>
            ...
            <button style={investigateBtnStyle} onClick={handleInvestigate}>Investigate</button>
          </>
        ) : (
          <>
            <div style={{ fontSize: 48, marginBottom: 16 }}>{typeIcons[outcome.type] ?? '\u2753'}</div>
            <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 12, color: typeColors[outcome.type] ?? '#e6edf3' }}>
              ...
              {outcome.type === 'heal' && `Team healed ${Math.round((outcome.healPercent ?? 0.3) * 100)}% HP!`}
              {outcome.type === 'damage' && `Team took ${Math.round((outcome.damagePercent ?? 0.15) * 100)}% HP damage!`}
              {outcome.type === 'champion_recruit' && (outcome.championId
                ? `${championDB.getById(outcome.championId)?.name ?? outcome.championId} joined your team!`
                : 'No one appeared...')}
              {outcome.type === 'stat_boost' && `+${outcome.statBoost?.amount ?? 0} ${(outcome.statBoost?.stat ?? 'STAT').toUpperCase()}`}
            ...
            {(outcome.type === 'heal' || outcome.type === 'damage') && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 24, width: '100%', maxWidth: 350 }}>
                {team.map(member => {
                  const champ = championDB.getById(member.championId);
                  const maxHp = champ?.stats.hp ?? 100;
                  const currentHp = member.currentHp ?? maxHp;
                  ...
                  return (
                    <div key={member.championId} style={memberRowStyle}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#e6edf3', marginBottom: 3 }}>
                        {champ?.name ?? member.championId}
                      </div>
                      <div style={hpBarBg}>
                        <div style={{ ...hpBarFill, width: `${pct}%`, background: pct < 30 ? '#ef4444' : pct < 60 ? '#facc15' : '#22c55e' }} />
                      </div>
                      <div style={{ fontSize: 10, color: '#8b949e', marginTop: 2 }}>{currentHp} / {maxHp} HP</div>
                    </div>
                  );
                })}
              </div>
...
const investigateBtnStyle: React.CSSProperties = {
  padding: '14px 40px', background: '#f97316', color: '#fff',
...
const memberRowStyle: React.CSSProperties = {
  background: '#161b22', padding: '6px 10px', borderRadius: 6, border: '1px solid #1e2a3a', width: '100%',
};
