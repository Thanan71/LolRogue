...
// Historical attempts must keep using the exact authority code that created them.
// @ts-ignore This immutable bundle is archived from the v1 gameplay release.
import { getAuthorityVerifier as getLegacyAuthorityVerifier } from './run-authority-v1.bundle.ts';
...
function resolveAuthorityVerifier(engineVersion: string, contentHash: string) {
  return (
    ...
    getLegacyAuthorityVerifier(engineVersion, contentHash)
  );
}
