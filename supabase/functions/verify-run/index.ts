...
Deno.serve(async (request) => {
  ...
  if (effectiveStatus.status !== 'finished') {
    return json(409, { error: 'run_attempt_not_sealed' });
  }
  ...
  const { data: claimData, error: claimError } = await admin.rpc(
    ...
    { p_attempt_id: attemptId, p_worker_id: crypto.randomUUID() } as never,
  );
  ...
  if (claim.claimed !== true) {
    return json(409, { error: 'verification_in_progress' });
  }
  if (
  ...
  const verifier = getAuthorityVerifier(claim.engine_version, claim.gameplay_content_hash);
  if (!verifier) return json(409, { error: 'unsupported_attempt_version' });
