-- Gameplay v12 unifies stat keys, calculation order and caps
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE version = 12
      AND code = '2026-08-canonical-stats-v12'
      AND engine_version = 'run-engine-v12'
      AND command_schema_version = 2
      AND content_hash =
        '2fa891690cfb0f1ae5ce4753de10ce690e3fb4163fde53b4c484984377eb2094'
  ) THEN
    RAISE EXCEPTION 'gameplay_ruleset_v12_contract_mismatch';
  END IF;
END
$$;
...
WHERE gameplay_ruleset_version = 11
...
SELECT
  12,
  '2026-08-authoritative-daily-v12',
  12,
  difficulty,
  'lolrogue.daily.v12',
  12,
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM public.daily_challenge_rulesets
    WHERE version = 12
      AND code = '2026-08-authoritative-daily-v12'
      AND gameplay_ruleset_version = 12
      AND seed_namespace = 'lolrogue.daily.v12'
      AND score_version = 12
  ) THEN
    RAISE EXCEPTION 'daily_ruleset_v12_contract_mismatch';
  END IF;
END
$$;
...
-- v12 changes canonical stat calculation. Reuse the audited v11 commit chain while
-- restoring the immutable v12 engine identity before returning.
ALTER FUNCTION public.complete_run_verification(UUID, UUID, JSONB, TEXT)
  RENAME TO complete_run_verification_v11_contract;
REVOKE ALL ON FUNCTION public.complete_run_verification_v11_contract(UUID, UUID, JSONB, TEXT)
...
BEGIN
  ...
  IF v_engine_version <> 'run-engine-v12' THEN
    RETURN public.complete_run_verification_v11_contract(
      p_attempt_id,
      p_lease_token,
      p_result,
      p_result_hash
  ...
  v_response := public.complete_run_verification_v11_contract(
    p_attempt_id,
    p_lease_token,
    p_result,
    p_result_hash
  ...
  UPDATE public.run_attempts
  SET engine_version = 'run-engine-v12'
  WHERE id = p_attempt_id;
