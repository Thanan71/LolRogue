-- Gameplay v14 fixes combat integrity across cooldowns, mana persistence,
-- composite targeting, rune triggers, executes and damage event ordering.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE version = 14
      AND code = '2026-08-combat-integrity-v14'
      AND engine_version = 'run-engine-v14'
      AND command_schema_version = 2
      AND content_hash = '486c5981a70dc84f5615af3abc4720ea03ce9085ae01595b2eda9bafc3f30708'
  ) THEN
    RAISE EXCEPTION 'gameplay_ruleset_v14_contract_mismatch';
  END IF;
END
$$;
...
SELECT 14, content_type, content_id, active, max_stacks
FROM public.gameplay_content_catalog
WHERE gameplay_ruleset_version = 13
...
UPDATE public.gameplay_rulesets SET is_active = TRUE WHERE version = 14;
...
SELECT
  14,
  '2026-08-authoritative-daily-v14',
  14,
  difficulty,
  'lolrogue.daily.v14',
  14,
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE version = 14
      AND code = '2026-08-authoritative-daily-v14'
      AND gameplay_ruleset_version = 14
      AND seed_namespace = 'lolrogue.daily.v14'
      AND score_version = 14
  ) THEN
    RAISE EXCEPTION 'daily_ruleset_v14_contract_mismatch';
  END IF;
END
$$;

UPDATE public.daily_challenge_rulesets SET is_active = TRUE WHERE version = 14;

ALTER FUNCTION public.complete_run_verification(UUID, UUID, JSONB, TEXT)
  RENAME TO complete_run_verification_v13_contract;
REVOKE ALL ON FUNCTION public.complete_run_verification_v13_contract(UUID, UUID, JSONB, TEXT)
...
BEGIN
  ...
  IF v_engine_version <> 'run-engine-v14' THEN
    RETURN public.complete_run_verification_v13_contract(
  ...
  v_response := public.complete_run_verification_v13_contract(
    p_attempt_id, p_lease_token, p_result, p_result_hash
  );
